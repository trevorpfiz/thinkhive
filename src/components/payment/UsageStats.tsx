        ...
        {stats &&
          stats.map((item, index) => (
            <Card key={index} className="mx-auto max-w-xs bg-gray-100 shadow-none">
              <Text>{item.name}</Text>
              {item.name === 'Credits' ? ( // conditionally render progress bar for "Credits"
                <div>
                  ...
                  <Flex className="mt-4 flex-col">
                    ...
                    <ProgressBar
                      percentageValue={Math.round((1 - item.stat / (item.limit as number)) * 100)}
                      className="mt-2"
                    />
                  </Flex>
                </div>
                ...
                <>
                  ...
                  <Flex className="mt-4 flex-col">
                    ...
                    <ProgressBar
                      percentageValue={Math.round((item.stat / (item.totalUsed as number)) * 100)}
                      className="mt-2"
                    />
                  </Flex>
                </>
